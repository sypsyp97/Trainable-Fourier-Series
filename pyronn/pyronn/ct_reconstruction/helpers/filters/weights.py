...
def cosine_weights_3d(geometry: Geometry):
    ...
    w = np.zeros(
        (geometry.detector_shape[-2], geometry.detector_shape[-1]), dtype=np.float32
    )
...
def parker_weights_2d(geometry: Geometry):
    ...
    angular_range = geometry.angular_range[-1]
    ...
    for beta_idx in range(weights.shape[0]):
        for gamma_idx in range(weights.shape[1]):
            ...
            gamma_angle = np.arctan(gamma_angle / (source_detector_distance + epsilon))

            # check if rays sampled twice and create weight volume
            if 0 <= beta <= 2 * (fan_angle - gamma_angle):
                val = (
                    np.sin(((np.pi / 4.0) * beta) / (fan_angle - gamma_angle + epsilon))
                    ** 2
                )
                if not np.isnan(val):
                    weights[beta_idx, gamma_idx] = val

            elif 2 * (fan_angle - gamma_angle) < beta < np.pi - 2 * gamma_angle:
                weights[beta_idx, gamma_idx] = 1.0

            elif np.pi - 2 * gamma_angle <= beta <= np.pi + 2 * fan_angle:
                val = (
                    np.sin(
                        (np.pi / 4.0)
                        * (
                            (np.pi + 2 * fan_angle - beta)
                            / (gamma_angle + fan_angle + epsilon)
                        )
                    )
                    ** 2
                )
...
def riess_weights_2d(geometry: Geometry):
    ...
    delta_x = geometry.angular_range[-1] - np.pi  # overscan
    ...
    def eta(beta, gamma_angle):
        return (
            np.sin(
                (np.pi / 2.0)
                * (np.pi + delta_x - beta)
                / (delta_x - 2 * gamma_angle + epsilon)
            )
            ** 2
        )

    def zeta(beta, gamma_angle):
        return np.sin((np.pi / 2.0) * beta / (delta_x + 2 * gamma_angle + epsilon)) ** 2

    weights = np.ones((geometry.number_of_projections, geometry.detector_shape[-1]))
    angular_increment = geometry.angular_range[-1] / geometry.number_of_projections
    beta = 0
    ...
    for beta_idx in range(weights.shape[0]):
        for gamma_idx in range(weights.shape[1]):
            # calculate correct pos on detector and current angle
            gamma_angle = (
                gamma_idx * geometry.detector_spacing[-1] + geometry.detector_origin[-1]
            )
            gamma_angle = np.arctan(
                gamma_angle / (geometry.source_detector_distance + epsilon)
            )

            if np.pi + 2 * gamma_angle <= beta <= np.pi + delta_x:
                val = eta(beta, gamma_angle)
                if not np.isnan(val):
                    weights[beta_idx, gamma_idx] = val

            if np.pi + 2 * (delta_x - gamma_angle) <= beta <= np.pi + delta_x:
                val = 2 - eta(beta, gamma_angle)
                if not np.isnan(val):
                    weights[beta_idx, gamma_idx] = val

            if 0 <= beta <= 2 * gamma_angle + delta_x:
                val = zeta(beta, gamma_angle)
                if not np.isnan(val):
                    weights[beta_idx, gamma_idx] = val

            if 0 <= beta <= -delta_x - 2 * gamma_angle:
    ...
    # additional scaling factor
    scale_factor = geometry.angular_range[-1] / np.pi
    return weights * scale_factor
