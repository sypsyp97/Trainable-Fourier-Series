...
def circular_trajectory_2d(number_of_projections, angular_range, **kwargs):
    ...
    for i in range(number_of_projections):
        if kwargs["swap_detector_axis"]:
            rays[i] = [
                np.cos(angular_range[0] + i * angular_increment),
                -np.sin(angular_range[0] + i * angular_increment),
            ]
        else:
            rays[i] = [
                np.cos(angular_range[0] + i * angular_increment),
...
def circular_trajectory_3d(
    ...
    **kwargs,
    ...
    for p in range(number_of_projections):
        ...
        extrinsic_params_mat = np.dot(
            np.dot(np.dot(translation, axis_align_R), R_x_axis), R_to_plane
        )
        extrinsic_params_mat = extrinsic_params_mat / extrinsic_params_mat[3, 3]

        # calculate projection matrix
        projection_matrices[p][0:3, 0:3] = np.dot(
            intrinsic_params_mat, extrinsic_params_mat[0:3, 0:3]
        )
        projection_matrices[p][0:3, 3] = np.dot(
            intrinsic_params_mat, extrinsic_params_mat[0:3, 3]
        )
